...
struct IEffectPass
{
	...
	// 获取输入签名
	// 在HLSL中，建议的语义顺序为
	// POSITION, NORMAL, TANGENT, COLOR, UV1, UV2, ..., UV8, CUSTOM_SEMANTICS...
	virtual const std::vector<D3D11_INPUT_ELEMENT_DESC>& GetInputSignatures() = 0;
	// 获取输入布局
...
public:
	...
	// 按名查找采样器状态并返回对应槽位，未找到则返回-1
	int GetSamplerStateByName(LPCSTR name);
	...
	// 按名查找采样器状态并返回对应槽位，未找到则返回-1
	int GetShaderResourceByName(LPCSTR name);
	...
	// 按名查找采样器状态并返回对应槽位，未找到则返回-1
	int GetUnorderedAccessByName(LPCSTR name);

	// 设置调试对象名
	void SetDebugObjectName(const std::string& name);


	void SetFrameUpdateFunc(std::function<void(Effect*, void*)> frameUpdateFunc);
	void SetGameObjectUpdateFunc(std::function<void(Effect*, GameObject*)> objectUpdateFunc);
	std::function<void(Effect*, void*)> GetFrameUpdateFunc();
	std::function<void(Effect*, GameObject*)> GetGameObjectUpdateFunc();

...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(DirectInput8Create(GetModuleHandle(nullptr), DIRECTINPUT_VERSION, IID_IDirectInput8, (void**)&s_DI, nullptr)))
            throw std::exception("DirectInput8 initialization failed.");

        if (FAILED(s_DI->CreateDevice(GUID_SysKeyboard, &s_Keyboard, nullptr)))
            throw std::exception("Keyboard CreateDevice failed.");
        if (FAILED(s_Keyboard->SetDataFormat(&c_dfDIKeyboard)))
            throw std::exception("Keyboard SetDataFormat failed.");
        if (FAILED(s_Keyboard->SetCooperativeLevel(s_hWnd, DISCL_FOREGROUND | (s_IsRelative ? DISCL_EXCLUSIVE : DISCL_NONEXCLUSIVE))))
            throw std::exception("Keyboard SetCooperativeLevel failed.");
        ...
        if (FAILED(s_Keyboard->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph)))
            throw std::exception("Keyboard set buffer size failed.");

        if (FAILED(s_DI->CreateDevice(GUID_SysMouse, &s_Mouse, nullptr)))
            throw std::exception("Mouse CreateDevice failed.");
        if (FAILED(s_Mouse->SetDataFormat(&c_dfDIMouse2)))
            throw std::exception("Mouse SetDataFormat failed.");
        if (FAILED(s_Mouse->SetCooperativeLevel(s_hWnd, DISCL_FOREGROUND | (s_IsRelative ? DISCL_EXCLUSIVE : DISCL_NONEXCLUSIVE))))
            throw std::exception("Mouse SetCooperativeLevel failed.");
#endif
...
void GameInput::Initialize(HWND hWnd, bool isRelative)
{
    // For Windows 8
    //    XInputEnable(TRUE);
    s_hWnd = hWnd;
    s_IsRelative = isRelative;
...
void GameInput::Update( float frameDelta )
{
    ...
    s_Analogs[kAnalogMouseX] = s_MouseState.lX;
    s_Analogs[kAnalogMouseY] = s_MouseState.lY;

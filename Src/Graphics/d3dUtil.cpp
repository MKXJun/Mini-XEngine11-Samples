...
std::wstring UTF8ToUCS2(std::string_view utf8Str)
{
	size_t byteCount = utf8Str.size();
	std::vector<wchar_t> res(byteCount + 1);
	MultiByteToWideChar(CP_UTF8, 0, utf8Str.data(), -1, res.data(), (int)byteCount + 1);
	return std::wstring(res.data());
}

std::string UCS2ToUTF8(std::wstring_view ucs2Str)
{
	size_t charCount = ucs2Str.size();
	std::vector<char> res(charCount * 2 + 1);
	WideCharToMultiByte(CP_UTF8, 0, ucs2Str.data(), -1, res.data(), (int)charCount * 2 + 1, nullptr, nullptr);
